...
Signed: 김영빈 Section: pset09 Student Number: 21600108
*/
...
pNode pop_backN(pNode p, int N) {
	...
	else
	{
		for(int i = 0; i < N; i++)
			p = pop_back(p);
		cout << setw(7) << "\r\tdeleting in [" << range - N << "]" << endl;
	}
...
pNode pop(pNode p, int val) {
	...
	pNode pop_node = p;
	pNode prev = nullptr;
	while(pop_node != nullptr)
	{
		if(pop_node->item == val)
		{
			pNode copy = pop_node->next;
			delete pop_node;
			prev->next = copy;
			return p;
		}
