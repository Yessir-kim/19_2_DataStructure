...
int main(int argc, char *argv[]) {
	...
	if (argc == 1) {
		...
		{
			cout << "Enter a number much larger than " << STARTING_SAMPLES << "." << endl;
			exit(-1);
		}
	}
	else if (argc == 2) {
		...
		{
			if(N == 0)
				cout << "Usage: profiling [N]" << endl;
			cout << "Enter a number much larger than " << STARTING_SAMPLES << "." << endl;
			exit(-1);
		}
		cout << "Enter the number of max entries to sort: " << N << endl;
	}
	else
	{
		cout << "Enter a number much larger than " << STARTING_SAMPLES << "." << endl;
  ...
  int *list = new (nothrow) int[N];                   // Use new & N here
...
/*	cout << "\n\tselectionSort(): already sorted" << endl;
	profiling(selectionSort, list, N, false);  // keep the data sorted

	cout << "\n\tselectionsort(): randomized" << endl;
	profiling(selectionSort, list, N);      // randomized by default

	cout << "\n\tselectionsort(): sorted reversed" << endl;
	int j = N - 1;
	for (int i = 0; i < N; i++) list[i] = j--;	// reversed sequence
	profiling(selectionSort, list, N, false);  // keep the data sorted
*/
