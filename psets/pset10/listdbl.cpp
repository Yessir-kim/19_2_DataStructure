...
Signed: 김영빈 Section: pset10 Student Number: 21600108
*/
...
pNode half(pNode lo, pNode hi) {
	...
	if(lo->next == hi)
	{
		if(hi->next->next == nullptr)
			return hi;
		else
			return lo;
	}
	pNode first = lo;
	while(first != hi && first != hi->next)
	{
		first = first->next->next;
		lo = lo->next;
	}
	// To use binary_search
	return lo;
	// return slow;
}
...
void push(pList p, int val, int x) {
	...
	pNode c = begin(p);
	for(; c!= end(p); c = c->next)
		if(c->item == x) break; // x location find

	insert(c,val); // Node connection
...
void reverse(pList p) {
	...
	while(c != nullptr) // end(p)도 가능할 듯 (이전 node에서 connection이 끝났기 때문에)
	{
		pNode temp_prev = c->prev;
		pNode temp_next = c->next;
		c->prev = c->next;
		c->next = temp_prev;
		c = temp_next;
	}
...
void shuffle(pList p) {
	...
	while(que != nullptr)
	{
		pNode temp_mid = mid->next;
		pNode temp_que = que->next;
		mid->next = que;
		que->prev = mid;
		que->next = temp_mid;
		temp_mid->prev = que;
...
bool sorted(pList p, int(*comp)(int a, int b)) {
	...
	for (pNode c = begin(p); c != end(p)->prev; c = c->next)
		if(comp(c->item,c->next->item) > 0) // not sorted
			return false;
...
void push_sortedNlog(pList p, int N) {
	...
	if(sorted(p,ascending)) quickSort(vals,N,ascending);
	else quickSort(vals,N,descending);

	pNode c = begin(p);
	int num = 0;
	while(c != end(p) && num != N - 1)
	{
		pNode prev = c;
		if(c->item > vals[num]) // 좀 잘 못 만들었네 .. ascending descending을 구분해서 만들어야 함 (flag 변수를 만들어서 check하면 가능할 듯)
		{
			insert(c,vals[num]);
			c = prev;
			num++;
