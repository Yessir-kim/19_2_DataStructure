...
Signed: 김영빈 Section: pset12 Student Number: 21600108
*/
...
bool containsIteration(tree node, int key) { // maybe BT is not acceptable
	if (empty(node)) return false;
	while (node) {
		if (key == node->key) return true;
...
tree findBT(tree t, int key) {
	...
	if (node != nullptr) return node; // 먼저 node->left를 보고 만약 nullptr이 아니라는 것은 key를 찾았다는 뜻임 따라서 return해주면 끝 만약 nullptr이면 node->right 탐색 
...
tree grow(tree node, int key) { // if it is BST 
...
tree trim(tree node, int key) {
	...
	else { // exist three case (case 1 : no child / case 2 : only one child / case 3 : two childs)
...
tree trimplus(tree node, int key) {
	...
	if (key < node->key) // if node to trim is in left subtree.
		...
		if (node->left && node->right) { // node->left와 node->right의 height 차이에 따라서 succ or pred 사용 
			if (height(node->left) <= height(node->right))
			{
				int succ = value(minimum(node->right));
				node->key = succ;
				node->right = trimplus(node->right, succ);
			}
			else
			{
				int pred = value(maximum(node->left));
				node->key = pred;
				node->left = trimplus(node->left, pred);
			}
			// get the successor: smallest in right subtree
			// copy the successor's content to this node
			// trim the successor recursively starting at root->right
...
void inorder(tree node, std::vector<tree>& v)
{
	DPRINT(cout << ">inorder size=" << v.size() << endl;);
	if (empty(node)) return;
	inorder(node->left, v);
...
void levelorder(tree node, vector<int>& v) {
	...
	while (!que.empty())
	{
		tree val = que.front();
		v.push_back(val->key);
		if (!empty(val->left)) que.push(val->left);
		if (!empty(val->right)) que.push(val->right);
		que.pop();
	}
...
bool _isBST(tree x, int min, int max) { // use another solution
	if (empty(x)) return true;
	DPRINT(cout << ">_isBST key=" << x->key << "\t min=" << min << " max=" << max << endl;);
	vector<int> v;
	int i = 0;
	int v_size = size(x) - 1; // exclude root number 
	inorder(x, v);
	for (; i < v_size; i++)
	{
		if (v[i] < min || v[i] > max) break;
		if (v[i] > v[i + 1]) break;
	}
	DPRINT(cout << "<_isBST key=" << x->key << "\t min=" << min << " max=" << max << endl;);
	if (i == v_size) return true;
	else return false;
}
...
bool _isBSTinOrder(tree root, tree *prev) {
	...
	if (!_isBSTinOrder(root->left, prev)) return false; // root->left 는 항상 root보다 작아야 되므로

	// Allows only distinct valued nodes
	if (*prev != nullptr && root->key <= (*prev)->key) return false;
	*prev = root;
	return _isBSTinOrder(root->right, prev); // prev값은 root임 root->right는 root보다 항상 작아야함 
}
...
tree rebalanceTree(tree node) {
	...
	vector<int> v;
	inorder(node, v);
	tree new_node = clear(node);
	new_node = buildAVL(v.data(), v.size());
...
tree buildAVL(tree* v, int n) {
	...
	root->right = buildAVL(v+mid+1, n - mid - 1);					// from mid+1 to the end (how many nodes?)
	return root;
}
*/
// rebuilds an AVL tree with a list of keys sorted.
