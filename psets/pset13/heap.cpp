...
Signed: ±è¿µºó Section: pset13 Student Number: 21600108
*/
...
void trim(heap p) {
	...
	if (size(p) == capacity(p) / 4) reserve(p, capacity(p) / 2);

	swap(p, 1, p->N--); // if heap ordered, root delete 
	sink(p, 1);
...
void growN(heap p, int count, bool heapOrdered) {
	...
	int* rannum = (int *)malloc(count * sizeof(int));
	randomN(max, count, rannum);
	for(int i = 0; i < count; i++)
		insertFunc(p, rannum[i]);
